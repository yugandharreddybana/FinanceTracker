...
import { formatCurrency, formatAxisMoney } from '../lib/utils';
import { compoundNetWorth, estimateNetWorthHistory } from '../lib/financeHelpers';
...
export function NetWorthPage() {
  const { bankAccounts, investments, loans, transactions, getNetWorth, userProfile } = useFinance();
  const [assumedGrowth] = useState(0.06);

  const displayCurrency = userProfile.preferences?.currency || 'INR';
  ...
  const netWorthConverted = getNetWorth();
  const chartAnchor =
    bankAccounts.some(a => (a.currency || 'INR') === displayCurrency) ||
    investments.some(i => (i.currency || 'INR') === displayCurrency) ||
    loans.some(l => (l.currency || 'INR') === displayCurrency)
      ? bankAccounts.filter(a => (a.currency || 'INR') === displayCurrency).reduce((s, a) => s + (a.type === 'Credit' ? -Math.abs(a.balance) : a.balance), 0) +
        investments.filter(i => (i.currency || 'INR') === displayCurrency).reduce((s, i) => s + i.quantity * i.currentPrice, 0) -
        loans.filter(l => (l.currency || 'INR') === displayCurrency).reduce((s, l) => s + l.remainingAmount, 0)
      : netWorthConverted;
  ...
  const history = useMemo(
    () =>
      estimateNetWorthHistory({
        transactions,
        currentNetWorth: chartAnchor,
        currency: displayCurrency,
        months: 6,
      }),
    [transactions, chartAnchor, displayCurrency]
  );
  ...
  const projections = useMemo(
    () =>
      [5, 10, 20].map((years) => ({
        years,
        value: compoundNetWorth(netWorthConverted, assumedGrowth, years),
      })),
    [netWorthConverted, assumedGrowth]
  );

  const hasTxHistory = transactions.some((t) => (t.currency || 'INR') === displayCurrency);

  return (
    <div data-testid="page-networth" className="p-4 md:p-8 max-w-[1400px] mx-auto space-y-6">
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}
        ...
        <div className="relative flex items-center gap-5 mb-8">
          ...
          <div><p className="text-lg text-slate-400 font-medium">Total Net Worth</p><p className="text-4xl md:text-5xl font-black mt-1">{formatCurrency(netWorthConverted, displayCurrency)}</p><p className="text-xs text-slate-500 mt-2 max-w-xl">Totals combine EUR and INR holdings using live cached FX rates (approximate).</p></div>
        </div>
        <div className="relative grid grid-cols-2 gap-4">
          <div className="rounded-2xl bg-white/5 border border-white/10 p-5">
            <p className="text-sm text-slate-400 font-medium">Total Assets</p>
            <p className="text-2xl font-black mt-1 text-emerald-400">{formatCurrency(totalAssets, displayCurrency)}</p>
          </div>
          <div className="rounded-2xl bg-white/5 border border-white/10 p-5">
            <p className="text-sm text-slate-400 font-medium">Total Liabilities</p>
            <p className="text-2xl font-black mt-1 text-rose-400">{formatCurrency(totalLiab, displayCurrency)}</p>
          </div>
        </div>
      </motion.div>
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }}
        ...
        <h3 className="font-bold text-slate-900 mb-1">Net Worth Trend</h3>
        <p className="text-sm text-slate-400 mb-4">
          {hasTxHistory
            ? `Estimated from your ${displayCurrency} cash flows (not stored historical snapshots).`
            : `Add ${displayCurrency} transactions to estimate how net worth moved month to month.`}
        </p>
        <ResponsiveContainer width="100%" height={280}>
          <AreaChart data={history}>
            ...
            <YAxis stroke="#94A3B8" fontSize={12} tickLine={false} axisLine={false} tickFormatter={(v) => formatAxisMoney(Number(v), displayCurrency)} />
            <Tooltip formatter={(v) => formatCurrency(Number(v), displayCurrency)} contentStyle={{ background: 'rgba(15,23,42,0.9)', border: 'none', borderRadius: 12 }} itemStyle={{ color: '#fff' }} />
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }}
          ...
          <div className="flex items-center gap-6">
            <ResponsiveContainer width="45%" height={180}>
              ...
              </Pie><Tooltip formatter={(v) => formatCurrency(Number(v), displayCurrency)} contentStyle={{ background: 'rgba(15,23,42,0.9)', border: 'none', borderRadius: 12 }} itemStyle={{ color: '#fff' }} /></PieChart>
            </ResponsiveContainer>
            <div className="space-y-4">
              <div className="flex items-center justify-between gap-4">
                ...
                <span className="font-bold text-slate-900">{formatCurrency(cash, displayCurrency)}</span>
              </div>
              <div className="flex items-center justify-between gap-4">
                ...
                <span className="font-bold text-slate-900">{formatCurrency(invTotal, displayCurrency)}</span>
              </div>
            </div>
          </div>
        </motion.div>
          ...
          <div className="flex items-center gap-6">
            ...
            <div className="space-y-4">
              <div className="flex items-center justify-between gap-4">
                ...
                <span className="font-bold text-rose-500">{formatCurrency(creditDebt, displayCurrency)}</span>
              </div>
              <div className="flex items-center justify-between gap-4">
                ...
                <span className="font-bold text-rose-500">{formatCurrency(loanDebt, displayCurrency)}</span>
              </div>
            </div>
          </div>
        </motion.div>
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.5 }}
        ...
        <p className="text-sm text-slate-400 mb-6">Illustrative compound growth at {(assumedGrowth * 100).toFixed(0)}% per year — not advice or a guarantee.</p>
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          {projections.map((p, i) => (
            <motion.div key={p.years} whileHover={{ y: -4 }}
              ...
              <p className="text-sm font-semibold text-violet-600">In {p.years} Years</p>
              <p className="text-2xl font-black text-slate-900 mt-2">{formatCurrency(p.value, displayCurrency)}</p>
              <p className="text-xs text-slate-400 mt-1">
                {netWorthConverted !== 0 ? `+${((((p.value - netWorthConverted) / Math.abs(netWorthConverted)) * 100) || 0).toFixed(0)}% vs today` : '—'}
              </p>
            </motion.div>
          ))}
        </div>
